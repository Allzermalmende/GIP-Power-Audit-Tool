...
function App() {
  ...
  // Load Stage 1 data (no header row) from Read sheet
  ...
  // Proceed to Stage 2, using range A1:M
  async function proceedToStage2() {
    ...
    try {
      ...
      const newRows = [];
      data.forEach(r => {
        const cab = r[0]; for (let c=1; c<=12; c++) if (r[c]) newRows.push({ cabinet: cab, loc: locMap[c], label: r[c], amperage:'', issue:false, info:'', extra:'' });
      });
      setRows(newRows);
      setStage(2);
    } catch (e) {
  ...
  // Submit Stage 2, update write sheet at A1:A
  async function submitAudit() {
    if (!confirm('Finish audit?')) return;
    const now = new Date(), ds = now.toISOString().slice(0,10);
    const csvName = `Power Audit ${ds} ${walkthrough}.csv`;
    ...
    try {
      await window.gapi.client.drive.files.create({resource:{name:csvName,mimeType:'text/csv',parents:[DRIVE_FOLDER_ID]},media:{mimeType:'text/csv',body:csv}});
      const sheets = window.gapi.client.sheets.spreadsheets.values;
      const head = await sheets.get({ spreadsheetId: BREAKDOWN_SHEET_ID, range: `${BREAKDOWN_WRITE}!1:1` });
      const hdrRow = head.result.values[0] || [];
      const colIdx = hdrRow.indexOf(walkthrough);
      if (colIdx < 0) throw 'Walkthrough not found';
      const sec = await sheets.get({ spreadsheetId: BREAKDOWN_SHEET_ID, range: `${BREAKDOWN_WRITE}!A1:A` });
      const secList = sec.result.values.map(r=>r[0]);
      ...
      alert('Audit saved!'); setStage(1); setWalkthrough(''); setSection(''); setUserName('');
